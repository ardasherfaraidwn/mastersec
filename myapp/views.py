...
def random_text():
    l = ['I used a qtip to clean my ears','He is a show off','Be very careful','I stated an opinion on what happened','His style is aggressive','Dont take her kindness for weakness','Bruce has a great reputation','The show is about to begin','He is quite skillful at playing basketball','I am thinking positively about the future']
    var = random.choice(l)
    return var
    # return pass
...
def home(request):
    if request.method == "POST":
        ...
        if request.POST.get('type') == "encode":
            ...
            context = {'data': result, 'text': text, 'valu': shift}
        elif request.POST.get('israndom') == "true":
            rand = random_text()
            text = rand
            context = {'text': text, 'valu': shift}
...
def affine(request):
    if request.method == "POST":
        ...
        if request.POST.get('type') == "encode":
            ...
            rand = random_text()
            text = rand
            context = {'text': text, 'valu1': key1, 'valu2': key2}
...
def decryptIrregular(cipher,key):
    ...
    try:
        ...
        return("This program cannot handle repeating words.")
